...
const Sidebar = () => {
  ...
  return (
    <aside className={`bg-black text-gold h-screen transition-all duration-300 ${open ? "w-64" : "w-16"} shadow-lg fixed z-50`}>
      <div className="flex items-center justify-between px-4 py-4 border-b border-gold/20">
        <span className={`text-xl font-bold transition-opacity ${open ? "opacity-100" : "opacity-0 w-0 overflow-hidden"}`}>Panel</span>
      ...
      <nav className="mt-6">
        {menuItems.map((item, index) => (
          <button
            ...
            onClick={() => alert(`\"${item.label}\" estará disponible próximamente 🚧`)}
          >
            {item.icon}
            <span className={`transition-opacity ${open ? "opacity-100" : "opacity-0 w-0 overflow-hidden"}`}>{item.label}</span>
          </button>
        ))}
      </nav>
    </aside>
